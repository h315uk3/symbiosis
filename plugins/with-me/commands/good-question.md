...
- `focus_areas`: Key topics to probe (use these to generate questions, NOT as answer options)
- `hypothesis_ids`: Internal IDs for likelihood estimation only (NOT answer options)
- `supports_multi_select`: Whether this dimension allows multiple selections (true/false)

**WARNING:** `hypothesis_ids` and `focus_areas` are internal data for question generation and Bayesian computation. They are NOT user-facing answer options. You must generate answer options independently in Step 2.2c.
...
- Dimension's focus areas from CLI output (Step 2.1) — use as context themes, NOT as answer options
...
**⛔ MANDATORY RULE:** The answer templates above are HYPOTHETICAL — for EIG calculation ONLY. They are NOT the user's actual answer. You MUST NOT use them as answers or pass them to `update-with-computation`. You MUST proceed to Step 2.2c and call AskUserQuestion to get the user's real response.
...
**⛔ MANDATORY CHECKPOINT — AskUserQuestion REQUIRED ⛔**

You have completed internal evaluation. The next action you MUST take is calling the `AskUserQuestion` tool. Do NOT call `update-with-computation`, do NOT estimate likelihoods, do NOT proceed to Step 2.3 until the user has answered via AskUserQuestion. Any answer templates from EIG estimation above are DISCARDED at this point — they were calculation artifacts only.
...
**⛔ PRE-CONDITION:** This step requires a REAL user answer obtained via AskUserQuestion in Step 2.2c. If you have not called AskUserQuestion and received the user's response, GO BACK to Step 2.2c immediately. Hypothetical answer templates from EIG estimation are NOT valid answers.
